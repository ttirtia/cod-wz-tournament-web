<template>
  <layout-admin>
    <div v-if="notFound" class="flex flex-grow justify-center pt-12">
      <p>This is not the team you're looking for.</p>
    </div>
    <div v-else-if="team !== null" class="flex flex-col flex-grow mx-auto max-w-6xl">
      <router-link
        class="flex w-max text-indigo-500 text-left mb-4"
        :to="'/admin/tournaments/' + team.tournament.id"
      >
        <i class="ri-arrow-left-s-line mt-0.5 mr-2" />
        back to tournament</router-link
      >
      <div
        class="flex flex-grow flex-row w-full items-center text-left space-x-4"
      >
            ...
            >
              <div class="flex-grow flex-col mr-4">
                <div class="flex items-center">
                  ...
                  <svg
                    v-if="team.teamLeader && team.teamLeader.id === player.id"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
...
export default {
  ...
  beforeMount() {
    this.apiFindTeams({ id: this.$route.params.id }).then((data) => {
      ...
      this.notFound = typeof this.team === "undefined";
    });
  },
  methods: {
    ...mapActions([
